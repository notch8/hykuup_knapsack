...
- **Merge only.** Squash and rebase are disabled on all three branches. This keeps commit SHAs identical across branches so you can always tell whether a commit has reached a given environment. It also prevents the submodule pointer rollback problem described in the [Merge strategy](../README.md#merge-strategy) section.
...
- **Required labels.** Every PR to `main` needs one of: a semver label (`major-ver`, `minor-ver`, `patch-ver`), `dependencies`, or `ignore-for-release`. The label determines the release-note category and version bump.
...
1. **Draft created on staging promotion.** When code is merged into `staging`, release-drafter collects all PRs since the last release and groups them by label into a draft release. The version number is determined at this point; the git tag is created when the draft is published.
